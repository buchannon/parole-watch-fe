...
const nextReviewBgStyles: Record<Exclude<OffenderStatus, 'Approved'>, string> = {
  ...
  'Not in Parole Review': 'border-gray-200 bg-gray-100',
  Unknown: 'border-red-200 bg-red-100',
}

const nextReviewTextStyles: Record<Exclude<OffenderStatus, 'Approved'>, string> = {
  'In Parole Review': 'text-blue-800',
  'Not in Parole Review': 'text-gray-800',
  ...
  return (
    <div className="space-y-8">
      ...
      {normalizedStatus !== 'Approved' && (
        <div className={cn('flex items-baseline gap-3 rounded-lg border px-4 py-3', nextReviewBgStyles[normalizedStatus])}>
          {normalizedStatus === 'In Parole Review' ? (
            <span className={cn('text-lg font-semibold', nextReviewTextStyles[normalizedStatus])}>In Parole Review</span>
          ) : (
            <>
              <span className="text-xs font-medium uppercase tracking-wide text-gray-500">Next parole review</span>
              <span className={cn('text-lg font-semibold', nextReviewTextStyles[normalizedStatus])}>
                {formatMonthYear(offender.next_parole_review_date)}
              </span>
            </>
          )}
        </div>
      ...
      <section>
        ...
        <div className="mt-3 flex flex-wrap gap-8">
          <div>
            <dt className="text-xs font-medium uppercase tracking-wide text-gray-500">Profile</dt>
          ...
          <div>
            <dt className="text-xs font-medium uppercase tracking-wide text-gray-500">Parole details</dt>
            <dd className="mt-0.5 text-sm">
              {offender.parole_details_url ? (
                <a href={offender.parole_details_url} target="_blank" rel="noreferrer" className="text-blue-600 hover:underline">
                  View parole details
                </a>
              ) : (
                '—'
      ...
      <dl className="grid grid-cols-1 gap-x-8 gap-y-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <DetailField label="Next parole review" value={formatMonthYear(offender.next_parole_review_date)} />
